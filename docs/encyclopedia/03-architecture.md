...
| Port | Local adapter (today) | AWS adapter (planned) |
|---|---|---|
| `IDocumentStorageService` | `LocalDocumentStorageService` (filesystem) | S3 + KMS + presigned URLs (M42) |
...
The AWS branches **fail fast** with a clear exception until their milestone lands — a
misconfigured profile can never silently run with the wrong adapter.
