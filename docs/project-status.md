...
- Latest closed milestone: Milestone 42 - Documents To S3 (merged via PR #34).
- Current milestone: Milestone 43 - Real Async Messaging, complete on branch `feat/milestone-43-async-messaging`. See `docs/dev/milestone-43-async-messaging-design.md` and `docs/dev/milestone-43-async-messaging-learnings.md`. Fills the `INotificationPublisher` SNS seam; notifications from the outbox now publish to SNS → SQS + DLQ under `Platform:Profile=Aws`. Developed/tested against LocalStack (no AWS account).
- Previous milestone: Milestone 42 - Documents To S3 (first fully-wired Local⇄AWS document-storage adapter). See the Milestone 42 status/verification lines below.
...
- Recommended next milestone after M43: `Milestone 44 - Caching + Rate Limiting` (Redis cache-aside for rebuildable reads, ASP.NET Core rate limiting, security headers; LocalStack/local Docker). Detailed M44 plan is in `docs/dev/production-transformation-roadmap.md`. Phases M42–M44 stay local/LocalStack (no AWS bill); a real AWS account is first needed at M45 (Terraform foundation).
