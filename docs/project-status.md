...
- Current branch: `feat/milestone-32-platform-module-skeleton`. Work now flows through GitHub pull requests into a protected `main` (CI-gated); do not commit directly to `main`.
...
- Latest closed milestone: Milestone 31 - Notification Inbox Read Model Foundation.
- Current milestone: Milestone 32 - Platform & Module Skeleton + Local⇄AWS Deploy Switch (the first milestone of the production-transformation program), implemented on branch `feat/milestone-32-platform-module-skeleton`. See the Milestone 32 status/verification lines below.
...
- Recommended next milestone: `Milestone 33 - Notifications Module + Team Inbox`. Carve the Notifications context into `src/Modules/Notifications/{Domain,Application,Infrastructure}` with its own `NotificationsDbContext` inheriting `ModuleDbContext` and owning a `notifications` PostgreSQL schema (the first schema-per-module split, moving the existing notification inbox + outbox-dispatch-to-inbox path into the module), and add the deferred underwriter/binding **team inbox** (per-user read receipts) in its proper home. This is also where `ICurrentUser`/`IUnitOfWork` relocate into `Platform.Abstractions`. Keep DynamoDB, production email/SMS delivery, a preference center, real-time push, per-type templates, and messaging threads out of scope.
