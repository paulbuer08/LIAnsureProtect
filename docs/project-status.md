...
- Current branch: `feat/evidence-upload-and-notification-usability`, created from synchronized `main` after the Evidence follow-up, SignalR, and connection-pool-governance closeout. Its first verified slice adds removable drag-and-drop selection across Evidence and Claim uploads, bounded response histories, clearer downloads, non-color-only unread Notification styling, and regression coverage proving a pre-review follow-up does not repeat an initial required document. The current unmerged slice adds collapsed confirmation guidance, ordinary Submission-to-Quote navigation, split/validated Philippine respondent contacts, bounded text, five-currently-unread owner follow-ups, and exact persisted Underwriter acknowledgement with optimistic concurrency. See `docs/dev/evidence-upload-and-notification-usability-learnings.md` and `docs/dev/evidence-follow-up-governance-and-quote-navigation-learnings.md`.
