...
- Current branch: `feat/milestone-38-underwriting-evidence-documents`. Work now flows through GitHub pull requests into a protected `main` (CI-gated); do not commit directly to `main`.
...
- Latest closed milestone: Milestone 37 - Underwriting Evidence.
- Current milestone: Milestone 38 - Underwriting Evidence Documents (fourth slice of the Underwriting carve), planned on branch `feat/milestone-38-underwriting-evidence-documents`. See `docs/dev/milestone-38-underwriting-evidence-documents-design.md` and `docs/superpowers/plans/2026-07-01-milestone-38-underwriting-evidence-documents.md`.
- Previous milestone: Milestone 37 - Underwriting Evidence (third slice of the Underwriting carve). See the Milestone 37 status/verification lines below.
...
- Milestone 38 planning status: M38 is scoped to complete the evidence-document carve. It should move generic private document storage contracts to `Platform.Abstractions`, move the evidence scanner port and local scanner adapter into the Underwriting module, move `QuoteEvidenceDocument` and scan status into Underwriting Domain, move document metadata into `underwriting.quote_evidence_documents`, move upload/replacement/download/accept/review gates into module Application handlers, and delete the temporary M37 `IEvidenceRequestWriter` seam after no callers remain. The public API and React evidence-document behavior should stay stable.
- Recommended next milestone after M38: `Milestone 39 - Quoting Decision Boundary`. Do not force final quote approval/decline/adjust into Underwriting until the Quote aggregate boundary is addressed. The next slice should prepare or begin the Quoting carve around `Quote`, `QuoteUnderwritingReview`, rating attempts, and final underwriting decision commands, while keeping Underwriting as the operational evidence/referral context. The dispatcher integration-event decoupling remains a near-term follow-up (currently still tracked as M40 in the M37/M38 docs).
