...
- Manual-retest hardening batch (collecting only, 2026-07-16): approved future work is now accumulated in `docs/dev/manual-retest-hardening-batch-plan.md` rather than implemented one finding at a time. The first entry covers layered respondent-email validation: authoritative NXDOMAIN/null-MX rejection, safe handling of legal implicit-MX fallback, non-blocking provider-typo suggestions, explicit unverified/verified contact state, and challenge-based ownership verification. No runtime behavior, schema, dependency, or test has changed for this collected item.
