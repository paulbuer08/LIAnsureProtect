...
- Current branch: `feat/milestone-34-notifications-team-inbox`. Work now flows through GitHub pull requests into a protected `main` (CI-gated); do not commit directly to `main`.
...
- Latest closed milestone: Milestone 33 - Notifications Module (first bounded-context carve).
- Current milestone: Milestone 34 - Notifications Team Inbox (first feature built inside a carved module), implemented on branch `feat/milestone-34-notifications-team-inbox`. See the Milestone 34 status/verification lines below.
- Previous milestone: Milestone 33 - Notifications Module (first bounded-context carve). See the Milestone 33 status/verification lines below.
...
- Recommended next milestone: `Milestone 35 - Underwriting Module Carve`. Continue the strangler: lift the Underwriting context (referral operations, evidence requests/reviews, AI review, underwriting decisions — currently overloaded inside `src/LIAnsureProtect.Domain/Quotes` and the legacy Application/Infrastructure) into `src/Modules/Underwriting/{Domain,Application,Infrastructure}` with its own `UnderwritingDbContext` + `underwriting` schema, following the M33 carve pattern (idempotent projection seams, two-context migrations already plumbed). Because Underwriting is a write-heavy context, this is where `IUnitOfWork` likely relocates into `Platform.Abstractions`. Keep integration-event decoupling of the dispatcher for M40.
