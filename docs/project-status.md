...
- Current branch: `fix/reassessment-self-service-cooldown`, created from synchronized `main` after PR #67. It corrects reassessment governance so the original Quote does not start a cooldown, the first valid reassessment remains self-service and immediate, a post-success cooldown returns retry guidance without creating Underwriter work, and only rolling/lifetime count overflow enters manual review. See `docs/dev/quote-supersession-and-reassessment-governance-design.md` and `docs/dev/quote-supersession-and-reassessment-governance-learnings.md`.
- Quote supersession and reassessment governance (2026-07-16, current branch): Quote versions remain immutable, but only the latest pre-contract version is current. Earlier Evidence and Notification projections become historical without rewriting their workflow/read audit state; active queues and badges exclude them. Customers have nested Quote history and read-only historical Evidence; Underwriters have a pending reassessment approval queue. Durable base-version, cooldown, rolling/lifetime allowance, one-pending-request, and burst controls prevent unlimited provider and projection work. See `docs/dev/quote-supersession-and-reassessment-governance-design.md` and `docs/dev/quote-supersession-and-reassessment-governance-learnings.md`.
- Quote supersession and reassessment governance verification (2026-07-16): full Docker-backed local CI rebuilt with 0 warnings/0 errors, recreated PostgreSQL and Redis, applied all four migration histories, passed 219 Unit tests and 283 Integration tests plus three intentional external-service skips, passed clean frontend install/build/lint/all 111 tests and API smoke checks, created `TestResults/local-ci-20260716-031625.zip`, and cleaned up its containers, volume, and network. All four current DbContexts separately report no pending model changes.
- Reassessment self-service cooldown correction (2026-07-16, current branch): Quote version 1 is now explicitly the baseline rather than a successful reassessment. Version 2 can therefore be created immediately when controls genuinely change. Only version 2 or later starts the 30-minute cooldown; a too-soon attempt receives safe retry guidance and creates no provider attempt or pending Underwriter review. Rolling/lifetime count overflow continues to use the audited Underwriter queue.
- Reassessment self-service cooldown verification (2026-07-16): zero-warning solution build; 226 Unit tests; 284 standalone Integration tests plus 4 intentional opt-in skips; clean pending-model checks for all four DbContexts; frontend TypeScript, ESLint, production build, and all 111 tests green. Docker-backed local CI passed 226 Unit tests, 285 Integration tests plus 3 expected skips, a clean frontend install with zero vulnerabilities, frontend/API smoke gates, created `TestResults/local-ci-20260716-055648.zip`, and removed PostgreSQL/Redis resources.
- Manual-retest hardening batch (collecting only, 2026-07-16): approved future work is now accumulated in `docs/dev/manual-retest-hardening-batch-plan.md` rather than implemented one finding at a time. Collected entries cover layered respondent-email deliverability/ownership verification; subject-aware Notification acknowledgement from every exact resource entry path, including an event/projector safety net and a view watermark for projection races; capability-aware notification actions so Underwriting team evidence updates deep-link to authorized work; an independent Underwriting Evidence review queue so `Quoted` Evidence-required work and unread customer follow-ups remain reachable even when the Quote referral queue is empty; persistent team-capable Notification scope tabs with filter-aware empty states; and concise respondent-contact help that removes visible placeholder duplication while preserving explicit validation and accessibility. No runtime behavior, schema, dependency, or test has changed for these collected items.
- Future Broker organizations milestone (approved plan only, 2026-07-16): the current Broker role deliberately remains on the same user-owned Customer workflow for now, but that is not the target brokerage model. `docs/dev/broker-organizations-and-delegated-access-plan.md` separately specifies Broker firms, client organizations, memberships, onboarding approval, customer consent/operations verification, capability-specific delegated authority, revocation/audit, role-aware workspaces, and an incremental migration from `OwnerUserId` to client-organization ownership. This large milestone is explicitly excluded from the current manual-retest hardening implementation and requires Legal/Compliance decisions before code begins.
...
- **Phase 3 Claims context DELIVERED (2026-07-07):** the full post-bind claim lifecycle (CM1–CM8 + post-CM8 hardening), built in parallel on `feat/claims-context`, merged to `main` as one consolidation PR — decision changed from "merge after Phase 2" to "merge now" since it was complete, green, and has no Phase-2 dependency. New `src/Modules/Claims` (own `claims` schema, `ClaimsDbContext`, 5 additive migrations, fourth outbox source); FNOL → adjuster queue (M44.5 concurrency pattern) → info loop → scan-gated documents → reserves (confidential) → decision/settlement (three charter guardrails) → close; ClaimsAdjuster role activated with `Claims.File/Read/Respond/Adjudicate` policies + `claims-operations` team inbox; `features/claims` frontend slice; **server-authoritative roles via `GET /api/v1/me`** (SPA reads roles from the API, not the token — provider-neutral). ~200 backend + ~60 frontend tests, zero-warning gate held. Docs folded into the Tier-1 living docs (Encyclopedia Ch 12 + Ch 3/5/10, Build History Era 7); per-milestone records archived under `docs/claims/`. **Local action for anyone pulling: run `scripts/update-database.ps1` to apply the five `claims` migrations.**
- Current work (2026-07-11): the Customer/Broker Policy Journey is implemented on `feat/customer-broker-policy-journey`: personal-only notifications have no tabs; actions are subject-aware; Customer/Broker/Admin have owner-scoped policy list/detail APIs and pages; persisted contractual status is separate from computed coverage state; submission detail presents Submission, Quote, and Policy together; draft deletion and idempotent pre-contract withdrawal preserve audit boundaries; duplicate drafts warn without blocking legitimate multiple submissions. See `docs/dev/customer-broker-policy-journey-learnings.md`.
- Manual-retest follow-up (2026-07-11, local only): draft creation now returns an exact existing owned Draft unless the caller explicitly chooses to create another legitimate application; the frontend supplies a stable idempotency key, disables pending creation, and navigates to Draft detail; submission wording is draft-explicit; editable fields switch in place inside the Submission record; deletion uses an accessible styled modal; and a dedicated configurable per-caller draft-create limit backs up the existing global rate limiter. No schema or cross-context boundary changed.
- Manual-retest presentation follow-up (2026-07-11, local only): an app-wide audit identified seven true transient confirmations—Draft created/updated, Submission submitted/withdrawn, Quote accepted, Evidence response saved, and manual Underwriting action saved. All now use polite live-region status messages that remain readable for five seconds, fade over the final half-second (respecting reduced-motion preferences), and then unmount so they do not permanently consume page space. Durable policy/quote/evidence-document results, timelines, decisions, audit records, warnings, errors, and empty states remain persistent by design. The reusable confirmation modal now has a prominent information panel; Draft deletion explains why unsubmitted data may be removed and when Submitted audit history becomes non-deletable, while the applicable withdrawal flow now uses the same modal and explains status change versus record erasure.
- Manual-retest follow-up verification: the standard Debug whole-solution build passed with 0 warnings and 0 errors; full Debug tests passed with UnitTests 196 and IntegrationTests 258 plus 4 intentional service opt-in skips; all four EF contexts report no pending model changes; frontend TypeScript, ESLint, production build, and all 88 tests pass. Full Docker-backed local CI applied all four migration sets to fresh PostgreSQL and passed UnitTests 196, IntegrationTests 259 plus 3 intentional external-service skips, frontend build/lint/all 88 tests, artifact creation, and Docker cleanup. Latest artifact: `TestResults/local-ci-20260711-192630.zip`. The branch remains local-only with no PR while further manual findings are collated.
- Cyber-control assurance implementation (2026-07-12): customer quote generation now requires a named attestation and detailed implementation facts; five versioned control assertions deliberately separate self-report from verification; deterministic rules create risk-based Underwriting evidence through the custom outbox; human evidence decisions project back to a separate Quote assurance state and gate acceptance; advisory document plausibility findings remain non-authoritative; and pre-acceptance reassessment creates an immutable successor quote. Design and acceptance scenarios: `docs/dev/cyber-control-assurance-and-reassessment-design.md`.
- Cyber-control assurance verification (2026-07-12): zero-warning solution build passed; the standalone backend run passed UnitTests 203 and IntegrationTests 265 plus 4 intended opt-in skips; all four DbContexts report no pending model changes; frontend TypeScript, ESLint, production build, and all 89 tests pass. Docker-backed local CI then applied every migration to fresh PostgreSQL and passed UnitTests 203, IntegrationTests 266 plus 3 intentional external-service skips, frontend build/lint/all 89 tests, artifact creation, and Docker cleanup. Artifact: `TestResults/local-ci-20260712-050920.zip`.
- Customer error and notification hardening design (2026-07-13): approved behavior separates user-safe guidance from developer diagnostics through stable Problem Details codes and correlation IDs; reassessment gains dirty detection and cancellation; evidence requests gain cursor-paged summaries and exact owner-scoped detail; quote/evidence notifications gain immutable subject identity; and production logging/browser telemetry gain a privacy-safe CloudWatch deployment contract. Live AWS resources remain owned by the future Terraform milestone.
- Customer error and notification hardening implementation (2026-07-13, local branch; no PR yet): every frontend feature API now uses a Zod-validated shared Problem Details boundary; raw JSON/exception/provider text is removed from customer pages; unexpected API and React failures use safe copy plus a support ID; Production/Aws hosts emit structured JSON request outcomes. Reassessment can be cancelled and cannot be created until a control assertion changes. Evidence now uses a lightweight owner-scoped cursor-paged list plus `/evidence-requests/{id}` detail, and quote history uses `/submissions/{submissionId}/quotes/{quoteId}`. Outbox snapshots enrich quote/evidence inbox titles and route each notification to its exact historical subject without cross-context reads. See `docs/dev/customer-error-and-notification-hardening-learnings.md` and `docs/dev/production-observability-and-customer-errors-runbook.md`.
- Customer error and notification hardening verification (2026-07-14): `dotnet build LIAnsureProtect.slnx --no-restore` passed with 0 warnings and 0 errors; the standalone backend suite passed UnitTests 206 and IntegrationTests 268 plus 4 intentional service opt-in skips; `SubmissionDbContext`, `NotificationsDbContext`, `UnderwritingDbContext`, and `ClaimsDbContext` all report no pending model changes; frontend TypeScript, ESLint, production build, and all 96 tests pass. Full Docker-backed local CI applied all four migration sets to fresh PostgreSQL and passed UnitTests 206, IntegrationTests 269 plus 3 intentional external-service skips, frontend build/lint/all 96 tests, artifact creation, and Docker cleanup. Artifact: `TestResults/local-ci-20260714-003907.zip`. The branch remains local-only with no PR while further manual findings are collated.
- Role-aware search/navigation implementation (2026-07-14): every Submission now has an immutable, unique `SUB-{year}-{16 hex}` reference while its UUID remains the technical identity. Submissions, owner Evidence, Policies, My Claims, Underwriting referrals, Claims adjudication, and Notifications now expose contextual search plus only the filters meaningful to the role/page. Owner/team scoping happens before filtering; search cannot widen authorization. Deeper resource pages use semantic breadcrumbs and browser-local time display. Draft creation and Evidence response support dirty-aware discard; Evidence requests explicitly require a document, allow one optionally, or require narrative only. Migrations: `AddSubmissionReference` and `AddEvidenceRequestIdentityAndDocumentRequirement`. Full decisions and implementation map: `docs/dev/role-aware-search-navigation-and-form-safety-learnings.md`.
- Role-aware search/navigation verification (2026-07-14): `dotnet build LIAnsureProtect.slnx --no-restore` passed with 0 warnings and 0 errors; the standalone backend suite passed UnitTests 209 and IntegrationTests 273 plus 4 intentional service opt-in skips; all four current DbContexts reported no pending model changes; frontend TypeScript, ESLint, production build, and all 99 tests passed. Full Docker-backed local CI then recreated PostgreSQL, applied all four migration sets, passed UnitTests 209 and IntegrationTests 274 plus 3 intentional external-service skips, passed frontend build/lint/all 99 tests, created its artifact, and removed the container, volume, and network. Artifact: `TestResults/local-ci-20260714-171307.zip`.
- Evidence follow-up and notification-context implementation (2026-07-14): owner Evidence responses are now append-only history instead of mutable latest text. `Responded` + `NotReviewed` requests accept meaningful follow-ups containing a response, an `Other concerns` note, or a document; each records respondent name/title/email, optional phone, response kind, and document linkage. Automatically generated material-control requests are `Required`, including a migration correction for existing automatic rows. Exact owner and Underwriter detail reads expose contact, response, and document history without widening ownership or module boundaries. Notification snapshots include safe company/Submission-reference context; the inbox groups messages, and action links mark read before exact navigation. The header never polls: after a durable projection commits, the Worker sends a payload-free `NotificationsChanged` hint through Redis to the API's authenticated SignalR hub; React Query reloads authorized HTTP state. Claim links open exact owner/adjuster destinations. SignalR failure never poisons the outbox. API and Worker each use one validated shared Npgsql pool across all four DbContexts (40/20 default maximums). Design and rationale: `docs/dev/evidence-response-follow-up-and-notification-context-design.md`; implementation record: `docs/dev/evidence-response-follow-up-and-notification-context-learnings.md`.
- Evidence follow-up, SignalR, and pool-governance verification (2026-07-15): `dotnet build LIAnsureProtect.slnx --no-restore` passed with 0 warnings and 0 errors; the standalone backend suite passed UnitTests 213 and IntegrationTests 279 plus 4 intentional opt-in skips; `SubmissionDbContext`, `NotificationsDbContext`, `UnderwritingDbContext`, and `ClaimsDbContext` all reported no pending model changes; frontend TypeScript, ESLint, production build, and all 104 tests passed. Full Docker-backed local CI recreated PostgreSQL and Redis, applied all four migration sets including `AddEvidenceResponseHistoryAndContacts`, passed UnitTests 213 and IntegrationTests 280 plus 3 intentional external-service skips, passed frontend build/lint/all 104 tests and API smoke checks, created its artifact, and removed both containers, the database volume, and network. Artifact: `TestResults/local-ci-20260715-001120.zip`.
- Evidence follow-up governance verification (2026-07-15, local only): the current unmerged slice builds with 0 warnings/0 errors; passes 217 Unit tests and 279 Integration tests plus 4 intentional opt-in skips; has no pending changes in all four DbContexts; and passes frontend TypeScript, ESLint, production build, and all 110 tests. After Docker Desktop recovered from an earlier host-runtime startup failure, full Docker-backed local CI recreated PostgreSQL and Redis, applied all four migration sets including `AddEvidenceFollowUpGovernance`, passed 217 Unit tests and 280 Integration tests plus 3 intentional service skips, passed frontend build/lint/all 110 tests, created `TestResults/local-ci-20260715-212619.zip`, and removed both containers, the database volume, and network. No PR has been opened.
- Reassessment and Evidence validation follow-up (2026-07-15, local branch; no PR yet): server and browser now agree that a changed detailed control answer is a genuine reassessment difference, with semantic JSON comparison preventing property-order false positives. Contradictory high-level/detail combinations are explained beside the controls before submission. Evidence contact fields expose accessible inline email and Philippine phone errors, while custom ASP.NET validation attributes protect JSON and multipart callers and the Underwriting domain retains final normalization authority. Verification: zero-warning build; 219 Unit tests; 280 standalone Integration tests plus 4 intentional opt-in skips; all four EF pending-model checks clean; frontend type/lint/build and all 110 tests green. Docker-backed local CI passed 219 Unit tests, 281 Integration tests plus 3 expected skips, frontend and API smoke gates, created `TestResults/local-ci-20260715-233524.zip`, and removed PostgreSQL/Redis resources.
- Evidence upload and Notification usability follow-up (2026-07-15, local branch; no PR yet): every applicable upload form uses one removable drag-and-drop selection component; long owner and Underwriter Evidence response histories scroll inside a bounded accessible region; clean document downloads use a conventional hand pointer; optional-document explanatory copy is omitted without changing the Optional contract; unread Notifications use an amber border, background, and text pill; and a Required request's pre-review follow-up is explicitly tested without a repeated document. See `docs/dev/evidence-upload-and-notification-usability-learnings.md`.
- Evidence upload and Notification usability verification (2026-07-15): zero-warning solution build passed; standalone tests passed with UnitTests 213 and IntegrationTests 279 plus 4 intentional opt-in skips; all four DbContexts reported no pending model changes; frontend TypeScript, ESLint, production build, and all 108 tests passed. Docker-backed local CI then passed UnitTests 213, IntegrationTests 280 plus 3 intentional service skips, and all frontend gates before cleaning PostgreSQL and Redis. Artifact: `TestResults/local-ci-20260715-022002.zip`. Vitest now uses a four-worker clean-runner cap after unrestricted parallelism produced CPU-pressure timeouts and interleaved `userEvent` typing without any product failure.
- Policy-journey verification (2026-07-11): `dotnet build LIAnsureProtect.slnx --no-restore` passed with 0 warnings and 0 errors; standalone `dotnet test --no-build` passed with UnitTests 195 and IntegrationTests 256 plus 4 intentional service opt-in skips; `SubmissionDbContext`, `NotificationsDbContext`, `UnderwritingDbContext`, and `ClaimsDbContext` all reported no pending model changes; frontend TypeScript, ESLint, production build, and all 84 tests passed. Full Docker-backed local CI then applied all four migration sets to fresh PostgreSQL and passed UnitTests 195, IntegrationTests 257 plus 3 intentional service skips, frontend build/lint/all 84 tests, artifact creation, and Docker cleanup. Artifact: `TestResults/local-ci-20260711-010323.zip`.
- Walkthrough-hardening verification (2026-07-10): `dotnet build` passed with 0 warnings/0 errors; `dotnet test --no-build` passed with UnitTests 189 and IntegrationTests 246 + 4 opt-in skips; all four EF contexts reported no pending model changes; full Docker-backed local CI passed with UnitTests 189, IntegrationTests 247 + 3 service skips, frontend build/lint, and 77 frontend tests. Artifact: `TestResults/local-ci-20260710-132954.zip`.
...
- Pre-Phase-2 decisions (2026-07, recorded in `docs/dev/phase2-infrastructure-and-identity-decisions.md`): (1) **parallel tracks** — Phase 2 (M45+ Terraform/AWS) on `main` via normal flow; (2) **AWS-native edge** — Route 53 + CloudFront + AWS WAF + Shield + ACM; the domain `liansureprotect.com` remains registered at Cloudflare while DNS delegates to Route 53 in Phase 2; Cloudflare CDN/WAF are unused (redundant with the AWS edge); (3) **identity stays Auth0**, now using the verified `auth.liansureprotect.com` custom domain; (4) domain secured (auto-renew, 2FA, DNSSEC, restrictive SPF/DKIM/DMARC — migrate DNS records to Route 53 in Phase 2). The first manual Customer/Broker walkthrough is complete.
- Latest product-hardening slice: `Customer/Broker Policy Journey` — implemented as specified, except Policy cancellation is deliberately deferred until it can include tested domain audit, claims, and notification implications. A bound Policy has no delete endpoint.
- Next production-transformation milestone after the walkthrough slices: `Milestone 45 - Terraform Foundation` — start of Phase 2 and the first milestone needing a real AWS account (MFA'd account, billing alarm, non-root IAM identity as prep). M45 provisions the Route 53 hosted zone + NS delegation + ACM per the edge decision above.
- Phase 2 HTTPS/environment guardrail recorded after local manual testing: local development remains HTTP on `localhost:5223`; production's public endpoint is HTTPS/443 at CloudFront + ALB/Ingress, with TLS terminated by ACM-backed edge infrastructure and trusted forwarded headers processed before HTTPS redirection. The workload definition must explicitly set `ASPNETCORE_ENVIRONMENT=Production` (runtime behavior) and `Platform__Profile=Aws` (adapter selection) as independent settings. M47 must test the public redirect, proxy scheme propagation, untrusted forwarded-header rejection, and absence of redirect loops. See the Production environment and HTTPS contract in `docs/dev/production-transformation-roadmap.md`.
