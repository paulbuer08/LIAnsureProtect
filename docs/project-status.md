...
- Current branch: `feat/evidence-upload-and-notification-usability`, created from synchronized `main` after the Evidence follow-up, SignalR, and connection-pool-governance closeout. This manual-test follow-up adds removable drag-and-drop selection across all Evidence and Claim uploads, bounded owner and Underwriter response histories, clearer download affordances, non-color-only unread Notification styling, and regression coverage proving a pre-review follow-up does not repeat an initial required document. No persistence, API, module, outbox, SignalR, or pool boundary changed. See `docs/dev/evidence-upload-and-notification-usability-learnings.md`.
