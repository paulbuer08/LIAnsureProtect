...
- Current milestone: Milestone 21 - Notification And Outbox Publishing Foundation is implemented on branch `codex/milestone-21-notification-and-outbox-publishing-foundation`. It puts the existing transactional outbox to practical use through retry-safe provider-shaped notification publishing for quote and policy workflow events without adding production SNS/SQS, real email/SMS, inboxes, preferences, webhooks, or notification templates yet.
