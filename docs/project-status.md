...
- Current branch: `feat/milestone-35-underwriting-ai-review-module`. Work now flows through GitHub pull requests into a protected `main` (CI-gated); do not commit directly to `main`.
- Git history: hosted at `github.com/paulbuer08/LIAnsureProtect` (public) with full history. Per-commit SHAs are no longer tracked in docs (history was rewritten once to remove a former-employer commit email, and the GitHub repo is now the source of truth); use commit messages and `git log` as the reference. Milestones remain traceable by their `feat:` / `docs:` commit messages. The repository's protection rules, CI/CD, security features, and automation (labeler, Claude PR review, Dependabot, CodeQL) are documented in `docs/dev/github-repository-and-automation.md`. The approved next direction is a multi-milestone **production transformation program** (modular monolith of bounded contexts, AWS-native, Cyber-only focus) — see `docs/dev/production-transformation-roadmap.md`. Under it, Milestone 32 is the Platform & module skeleton + Local⇄AWS deploy switch, and the deferred team inbox reslots to Milestone 33 (Notifications module).
- Latest closed milestone: Milestone 34 - Notifications Team Inbox.
- Current milestone: Milestone 35 - Underwriting Module: AI Review (first slice of the Underwriting carve), implemented on branch `feat/milestone-35-underwriting-ai-review-module`. See the Milestone 35 status/verification lines below.
- Previous milestone: Milestone 34 - Notifications Team Inbox (first feature built inside a carved module). See the Milestone 34 status/verification lines below.
- Previous milestone: Milestone 31 - Notification Inbox Read Model Foundation is implemented on branch `feat/milestone-31-notification-inbox-read-model`. It adds an owner-facing notification inbox so customers/brokers can read the notifications the system already publishes: a PostgreSQL `notification_inbox_entries` read model behind the Application-owned `INotificationInboxRepository`, written by the outbox dispatcher beside the existing publish step (only for `customer-or-broker`-addressed messages, idempotent on the source outbox message id) within its existing save; owner-scoped `GET /api/v1/notifications` (list + unread count) and `POST /api/v1/notifications/{id}/read` behind the new `Notifications.Read` policy; and a React `features/notifications` slice (list page with unread count + mark-read) plus a dashboard entry point. Underwriter/binding "team inbox" audiences, DynamoDB, production email/SMS delivery, a preference center, real-time push, per-type templates, and messaging threads remain out of scope.
...
- Recommended next milestone: `Milestone 36 - Underwriting Referral Operations`. Continue the Underwriting carve with the next-safest slice: lift referral operations (the queue/SLA `QuoteReferralOperation` + work notes + follow-up tasks + timeline, currently in `Domain/Quotes` + the legacy Application/Infrastructure) into the Underwriting module + `underwriting` schema. The hand-off to solve: a referral operation is created when a quote is referred (Quoting → Underwriting), so introduce a port or event for that creation. Evidence (requests/documents/reviews) and the decision (`ApproveReferral`/`DeclineReferral`/`AdjustReferral` + `QuoteUnderwritingReview`) are later slices; the decision is the hardest (it stays on the Quote aggregate or moves via a Quoting command port) and is where `IUnitOfWork` likely relocates into `Platform.Abstractions`. Keep integration-event decoupling of the dispatcher for M40.
