...
- Manual-retest hardening batch (collecting only, 2026-07-16): approved future work is now accumulated in `docs/dev/manual-retest-hardening-batch-plan.md` rather than implemented one finding at a time. Collected entries cover layered respondent-email deliverability/ownership verification and subject-aware Notification acknowledgement from every exact resource entry path, including an event/projector safety net and a view watermark for projection races. No runtime behavior, schema, dependency, or test has changed for these collected items.
