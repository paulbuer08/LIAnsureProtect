...
- Current branch: `feat/evidence-follow-up-and-notification-context`, created from synchronized `main` commit `7a191b7` after the role-aware search/navigation closeout. The approved slice adds append-only pre-review Evidence follow-ups, verifiable respondent contact fields, corrected automatic document requirements, Submission-aware notification grouping, automatic read-on-open, payload-free SignalR badge invalidation over a Redis API/Worker backplane, exact Claim deep links, and explicit shared Npgsql pool governance. See `docs/dev/evidence-response-follow-up-and-notification-context-design.md` and `docs/superpowers/plans/2026-07-14-evidence-response-follow-up-and-notification-context.md`.
