...
- Current branch: `feat/evidence-follow-up-and-notification-context`, created from synchronized `main` commit `7a191b7` after the role-aware search/navigation closeout. The approved slice adds append-only pre-review Evidence follow-ups, verifiable respondent contact fields, corrected automatic document requirements, Submission-aware notification grouping, automatic read-on-open, and a lightweight eventually-consistent badge refresh. See `docs/dev/evidence-response-follow-up-and-notification-context-design.md` and `docs/superpowers/plans/2026-07-14-evidence-response-follow-up-and-notification-context.md`.
...
- Evidence follow-up and notification-context implementation (2026-07-14): owner Evidence responses are now append-only history instead of mutable latest text. `Responded` + `NotReviewed` requests accept meaningful follow-ups containing a response, an `Other concerns` note, or a document; each records respondent name/title/email, optional phone, response kind, and document linkage. Automatically generated material-control requests are `Required`, including a migration correction for existing automatic rows. Exact owner and Underwriter detail reads expose contact, response, and document history without widening ownership or module boundaries. Notification event snapshots now include safe company/Submission-reference context; the inbox groups messages by that context, and action links mark read before exact navigation. The header uses a dedicated unread-count query without continuous polling; it refreshes through meaningful cache invalidation, Notifications navigation, and window focus. Standalone `Mark as read` controls are removed. Design and rationale: `docs/dev/evidence-response-follow-up-and-notification-context-design.md`; implementation record: `docs/dev/evidence-response-follow-up-and-notification-context-learnings.md`.
- Evidence follow-up and notification-context verification (2026-07-14): `dotnet build LIAnsureProtect.slnx --no-restore` passed with 0 warnings and 0 errors; the standalone backend suite passed UnitTests 213 and IntegrationTests 274 plus 4 intentional opt-in skips; `SubmissionDbContext`, `NotificationsDbContext`, `UnderwritingDbContext`, and `ClaimsDbContext` all reported no pending model changes; frontend TypeScript, ESLint, production build, and all 101 tests passed. Full Docker-backed local CI recreated PostgreSQL, applied all four migration sets including `AddEvidenceResponseHistoryAndContacts`, passed UnitTests 213 and IntegrationTests 275 plus 3 intentional external-service skips, passed frontend build/lint/all 101 tests, created its artifact, and removed the container, volume, and network. Artifact: `TestResults/local-ci-20260714-213918.zip`.
