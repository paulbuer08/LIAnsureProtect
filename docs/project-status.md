...
- Current branch: `feat/role-aware-dashboard-navigation`, closing the first Customer/Broker manual-walkthrough hardening pass. Work flows through GitHub pull requests into a protected `main` (CI-gated); do not commit directly to `main`.
...
- Current work (2026-07-10): the first real Customer/Broker walkthrough hardening pass is complete on `feat/role-aware-dashboard-navigation`. It added server-authoritative role-aware navigation/route guards, a visible gitignored API `.env.local` convention, Auth0 consent/callback recovery, dashboard debug-panel removal, draft edit + submit actions, the complete submitted -> quote -> accept -> bind customer flow, Philippine-peso rating intake with contextual help/Other-industry/prior-incident detail, and durable latest-quote/idempotent-notification behavior. The session record and approved next plan are in `docs/dev/customer-broker-walkthrough-hardening-and-policy-journey-plan.md`.
...
- Next product-hardening slice: `Customer/Broker Policy Journey` — role-correct notification filters (no tabs for personal-only users), owner-scoped policy list/detail APIs and pages, policy-aware notification actions, separate submission/quote/policy state presentation, and audited draft-delete/withdrawal/cancellation rules. The approved phased plan is in `docs/dev/customer-broker-walkthrough-hardening-and-policy-journey-plan.md`.
