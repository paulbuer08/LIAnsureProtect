...
- Latest closed milestone: Milestone 43 - Real Async Messaging (merged via PR #35).
- Current milestone: Milestone 44 - Caching + Rate Limiting, complete on branch `feat/milestone-44-caching-rate-limiting`. See `docs/dev/milestone-44-caching-rate-limiting-design.md` and `docs/dev/milestone-44-caching-rate-limiting-learnings.md`. Adds the `ICacheService` cache-aside port (in-memory/Redis by profile), API rate limiting (429), and security headers. Last local/LocalStack milestone before Phase 2 (M45 Terraform foundation) needs a real AWS account.
- Previous milestone: Milestone 43 - Real Async Messaging (SNS → SQS + DLQ behind the notification publisher port). See the Milestone 43 status/verification lines below.
