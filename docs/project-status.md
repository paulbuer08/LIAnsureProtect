...
- Current branch: `feat/milestone-37-underwriting-evidence`. Work now flows through GitHub pull requests into a protected `main` (CI-gated); do not commit directly to `main`.
...
- Latest closed milestone: Milestone 36 - Underwriting Referral Operations.
- Current milestone: Milestone 37 - Underwriting Evidence (third slice of the Underwriting carve), implemented on branch `feat/milestone-37-underwriting-evidence`. See the Milestone 37 status/verification lines below.
- Previous milestone: Milestone 36 - Underwriting Referral Operations (second slice of the Underwriting carve). See the Milestone 36 status/verification lines below.
...
- Milestone 37 verification: `dotnet build LIAnsureProtect.slnx --no-restore` reported 0 warnings/0 errors; `dotnet test LIAnsureProtect.slnx --no-build` passed with UnitTests 62 and IntegrationTests 124, with the one PostgreSQL opt-in test skipped by design; `dotnet ef migrations has-pending-model-changes` reported no changes for `SubmissionDbContext`, `NotificationsDbContext`, and `UnderwritingDbContext`. Full local CI is the final pre-PR gate for this branch.
- Recommended next milestone: `Milestone 38 - Underwriting Evidence Documents`. Continue the Underwriting carve by moving document metadata/storage/scanning workflows out of legacy into the Underwriting module: `QuoteEvidenceDocument`, document upload/download/replacement handlers, scan gates, and document read models should move behind module-owned ports while preserving private storage, clean-only download/acceptance behavior, and audit history. The final quote underwriting decision (`ApproveReferral`/`DeclineReferral`/`AdjustReferral` + `QuoteUnderwritingReview`) remains a later slice because it is still owned by the Quote aggregate.
