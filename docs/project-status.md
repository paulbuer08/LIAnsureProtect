...
- Latest closed milestone: Milestone 35 - Underwriting Module: AI Review.
- Current milestone: Milestone 36 - Underwriting Referral Operations (second slice of the Underwriting carve), implemented on branch `feat/milestone-36-underwriting-referral-operations`. See the Milestone 36 status/verification lines below.
- Previous milestone: Milestone 35 - Underwriting Module: AI Review (first slice of the Underwriting carve). See the Milestone 35 status/verification lines below.
...
- Milestone 36 verification: `dotnet build LIAnsureProtect.slnx` 0 warnings/0 errors; `dotnet test LIAnsureProtect.slnx --no-build` passed with UnitTests 62 and IntegrationTests 118, 1 PostgreSQL opt-in test skipped; `dotnet ef migrations has-pending-model-changes` reported no model changes for all three contexts (Submission, Notifications, Underwriting). Full local CI to run before commit; delivered through the protected-`main` pull-request flow so GitHub Actions CI gates the merge.
- Recommended next milestone: `Milestone 37 - Underwriting Evidence`. Continue the Underwriting carve with the largest sub-context: lift evidence (requests/documents/reviews) into the Underwriting module + `underwriting` schema, and finish the evidence↔operation decoupling started in M36 (remove the now-vestigial `quote_referral_operation_id` column; the evidence aggregate references the operation by id/events only). The **decision** (`ApproveReferral`/`DeclineReferral`/`AdjustReferral` + `QuoteUnderwritingReview`) remains the hardest, latest slice — it stays on the Quote aggregate or moves via a Quoting command port, and is where `IUnitOfWork` likely relocates into `Platform.Abstractions` (M36 did **not** need that relocation; the module commits through its own DbContext). Keep integration-event decoupling of the dispatcher for M40.
