...
- Current milestone: Milestone 26 - Evidence Request Notification and Follow-up Foundation is started on branch `codex/milestone-26-evidence-request-notification-follow-up-foundation`. The recommended planning target is a realistic notification and follow-up workflow for evidence requests that reuses the existing local notification/outbox boundary, surfaces overdue evidence follow-up state, and keeps production email/SMS delivery, notification inboxes, full document storage, OCR, RAG, autonomous AI review, and full broker/customer messaging threads out of scope.
