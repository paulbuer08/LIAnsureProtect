...
Status: started on branch `codex/milestone-21-notification-and-outbox-publishing-foundation`.
...
- Add an Application-owned notification publisher boundary that can publish messages derived from durable outbox rows.
- Keep the local domain event/outbox model authoritative; notifications should be downstream side effects, not business decision makers.
- Add an Infrastructure local provider-shaped publisher so tests can prove publishing behavior without AWS credentials.
- Update Worker dispatch behavior so publishing, retry, and processed/failure metadata are explicit instead of only marking rows processed.
- Add focused message types for important quote and policy events such as quote generated, quote referred, quote underwriting decision recorded, and policy bound.
- Preserve idempotency and retry safety: one business action should not produce duplicate downstream notification messages.
- Add unit and integration tests proving publish success, transient failure retry, poison/failure recording, and migration/index shape if new persistence is needed.
