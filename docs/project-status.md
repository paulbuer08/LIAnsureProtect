...
- Current branch: `feat/milestone-33-notifications-module`. Work now flows through GitHub pull requests into a protected `main` (CI-gated); do not commit directly to `main`.
...
- Latest closed milestone: Milestone 32 - Platform & Module Skeleton + Local⇄AWS Deploy Switch.
- Current milestone: Milestone 33 - Notifications Module (first bounded-context carve), implemented on branch `feat/milestone-33-notifications-module`. See the Milestone 33 status/verification lines below.
- Previous milestone: Milestone 32 - Platform & Module Skeleton + Local⇄AWS Deploy Switch (the first milestone of the production-transformation program). See the Milestone 32 status/verification lines below.
...
- Recommended next milestone: `Milestone 34 - Notifications Team Inbox`. Persist the `underwriting-operations`/`binding-operations` (team) audiences the dispatcher currently drops, with **per-user read receipts** (created lazily on mark-read so no user directory is needed), role→audience visibility in the list query (via `ICurrentUser.IsInRole`), and a frontend surface for team notifications. This adds a second module-owned table (and a read-receipt aggregate) inside `src/Modules/Notifications`. Keep DynamoDB, production email/SMS delivery, a preference center, real-time push, per-type templates, and messaging threads out of scope.
