...
- Pre-Phase-2 decisions (2026-07, recorded in `docs/dev/phase2-infrastructure-and-identity-decisions.md`): (1) **parallel tracks** — Phase 2 (M45+ Terraform/AWS) on `main` via normal flow, while Phase 3 **Claims** builds autonomously on the long-lived `feat/claims-context` branch (own worktree) and does NOT merge to main until Phase 2 is done; (2) **AWS-native edge** — Route 53 + CloudFront + AWS WAF + Shield + ACM; the domain `liansureprotect.com` is registered at Cloudflare (registrar only) and DNS delegates to Route 53 in Phase 2; Cloudflare CDN/WAF are unused (redundant with the AWS edge); (3) **identity stays Auth0** (real swap decision is Auth0-vs-Cognito, not Cloudflare — Cloudflare is not an IdP); (4) domain secured (auto-renew, 2FA, DNSSEC, restrictive SPF/DKIM/DMARC — temporary, move to Route 53 in Phase 2). The owner is doing the first manual UI walkthrough (running-the-app + manual-testing guides) before M45.
- Next milestone: `Milestone 45 - Terraform Foundation` — start of Phase 2 and the first milestone needing a real AWS account (MFA'd account, billing alarm, non-root IAM identity as prep). M45 must provision the Route 53 hosted zone + NS delegation + ACM per the edge decision above.
