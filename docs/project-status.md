...
Status: started on branch `codex/milestone-19-external-rating-provider-adapter-and-resilience-foundation`.
...
- Add a provider-shaped external rating integration boundary.
- Keep the Application layer dependent on an interface, not an Infrastructure HTTP implementation.
- Add an Infrastructure HTTP adapter through `IHttpClientFactory`.
- Use a local fake provider endpoint or test handler first, not real insurer credentials.
- Add retry and circuit-breaker behavior around the external HTTP call.
- Persist provider attempt success/failure details safely.
- Preserve local rating and underwriting referral behavior when the external provider fails.
