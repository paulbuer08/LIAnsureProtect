...
- Recommended next milestone after M41: `Milestone 42 - Documents To S3`. Replace the local private document byte-storage adapter with an S3-shaped adapter while preserving evidence document metadata, scan trust state, and clean-only download gates.
