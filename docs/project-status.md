...
- Current milestone: Milestone 27 - Evidence Document Storage Foundation is started on branch `codex/milestone-27-evidence-document-storage-foundation`. The recommended planning target is to replace evidence response attachment metadata placeholders with a narrow local document-storage boundary for uploaded evidence files, private download/read access, and audit-friendly storage metadata while keeping production S3, virus scanning, OCR, embeddings, RAG, autonomous AI document review, and full document management out of scope for the first storage slice.
