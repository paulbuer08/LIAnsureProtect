...
- Current branch: `feat/customer-error-and-notification-hardening`, created from synchronized `main` after PR #63 merged the cyber-control assurance and reassessment milestone. The active product-hardening slice centralizes safe customer errors, makes reassessment cancellable, gives each evidence request and quote notification an exact destination, replaces the eager evidence wall with paged summaries plus owner-scoped detail, and defines the production CloudWatch/RUM contract without provisioning AWS resources ahead of Terraform. See `docs/dev/customer-error-and-notification-hardening-design.md` and `docs/superpowers/plans/2026-07-13-customer-error-and-notification-hardening.md`.
