...
Milestone 38 is planned as `Milestone 38 - Underwriting Evidence Documents`, the fourth Underwriting carve slice. It completes the M37 temporary seam by moving evidence document metadata, storage/scanning orchestration, replacement uploads, clean-only download/review gates, and document-aware owner reads into the Underwriting module. The plan first moves generic private document storage contracts into Platform.Abstractions and moves the evidence scanner port into Underwriting, so the module can own document workflows without referencing legacy Application. See [Milestone 38 Design](docs/dev/milestone-38-underwriting-evidence-documents-design.md) and [Milestone 38 Plan](docs/superpowers/plans/2026-07-01-milestone-38-underwriting-evidence-documents.md).
