...
- Reassessment comparison now treats both high-level control states and semantically compared detailed implementation answers as version-worthy changes. The UI explains unchanged answers and blocks internally contradictory combinations locally, while the Application validator remains authoritative. Evidence response contact inputs now show accessible inline email/Philippine-mobile/Philippine-landline errors instead of silently disabling submission; custom ASP.NET validation attributes protect JSON and multipart API callers as well.
- Reassessment and Evidence validation verification passed with a zero-warning solution build; 219 standalone unit tests; 280 standalone integration tests plus 4 intentional service skips; four clean EF pending-model checks; and frontend TypeScript, ESLint, production build, and all 110 tests. Docker-backed local CI applied all four migration sets, passed 219 unit tests, 281 integration tests plus 3 intentional service skips, frontend build/lint/all 110 tests and API smoke checks, then cleaned PostgreSQL and Redis. Artifact: `TestResults/local-ci-20260715-233524.zip`.
- Progressive confirmation guidance now starts collapsed behind an accessible `More details` disclosure across the shared modal. Submission detail links its latest Quote card to the exact immutable nested Quote route, so Quote history is no longer reachable only through Notifications.
- Evidence follow-up governance now treats changed respondent contact details as an auditable follow-up, separates optional Philippine mobile and telephone numbers with canonical validation, and enforces server-side limits for respondent and narrative fields. Owners may have five currently unread follow-ups; an Underwriter opens one exact entry through an idempotent audited command, restoring one slot. `AddEvidenceFollowUpGovernance` adds split contacts, response view metadata, an unread-count index, safe legacy-phone backfill, and request optimistic concurrency.
- Evidence follow-up governance verification passed with a zero-warning solution build; 217 unit tests; 279 standalone integration tests plus 4 intentional service skips; four clean EF pending-model checks; and frontend TypeScript, ESLint, production build, and all 110 tests. Docker-backed local CI applied all four migration sets including `AddEvidenceFollowUpGovernance`, passed 217 unit tests, 280 integration tests plus 3 intentional service skips, and all 110 frontend tests before cleaning PostgreSQL and Redis. Artifact: `TestResults/local-ci-20260715-212619.zip`.
- File-selection usability across Evidence responses, Evidence replacements, and Claim documents: supported files can be chosen or dragged onto the form, every pending file has an accessible remove action, duplicate/unsupported/over-limit choices receive client feedback, and submitted audit documents remain immutable. Owner and Underwriter response histories now use bounded keyboard-scrollable regions; clean document downloads show the conventional link pointer.
- Clearly distinguishable unread Notifications now use an amber border and background plus a visible `Unread` pill, rather than relying on a subtle background shade alone. A Required Evidence request still requires its initial document, while a meaningful pre-review follow-up is explicitly allowed and regression-tested without uploading that document again.
- Evidence upload and Notification usability verification passed with a zero-warning solution build; 213 unit tests; 280 Docker-backed integration tests plus 3 intentional service skips; four clean EF pending-model checks; and frontend TypeScript, ESLint, production build, and all 108 tests. Local CI artifact: `TestResults/local-ci-20260715-022002.zip`.
- Auditable pre-review Evidence follow-ups: an owner may add another response, concern, or document while a request is `Responded` and its review decision is still `NotReviewed`. Every submission is an immutable `quote_evidence_responses` row linked to its documents; respondent name, title, and valid email are required, while phone and `Other concerns` are optional. The original response is never overwritten.
- Evidence trust and notification context hardening: automatically projected material-control requests are corrected to `Required`; owner and Underwriter detail views expose the exact response/contact/document history; notification snapshots carry company and Submission reference; the inbox groups related messages; and actionable notification links mark themselves read. Standalone `Mark as read` controls and continuous badge polling are removed; the lightweight unread count loads with the signed-in shell and refreshes on meaningful cache invalidation, Notifications navigation, and window focus.
- Realtime notification invalidation without business-data push: an authenticated `NotificationHub` receives payload-free `NotificationsChanged` hints over a Redis backplane after the Worker commits an inbox projection. The React shell reconnects automatically and invalidates TanStack Query inbox/count state; focus/navigation remains a safety net. Claim notifications now deep-link to exact personal or operational Claim destinations. Redis/SignalR failure is logged but cannot poison the durable outbox.
- Explicit PostgreSQL connection-pool governance: API and Worker each register one shared `NpgsqlDataSource` across Submission, Notifications, Underwriting, and Claims contexts, with startup-validated maximums (40 API / 20 Worker), acquisition/command timeouts, idle pruning, lifetime, and host application names. RDS Proxy/PgBouncer and data-model scaling stay measured deployment decisions.
- Evidence follow-up, SignalR, and pool-governance verification passed with a zero-warning solution build; 213 unit tests; 280 Docker-backed integration tests plus 3 intentional external-service skips; four clean EF pending-model checks; and frontend TypeScript, ESLint, production build, and all 104 tests. Local CI artifact: `TestResults/local-ci-20260715-001120.zip`.

- Immutable human Submission references (`SUB-{year}-{16 hex}`) with safe legacy backfill and a unique database constraint. UUIDs remain the technical primary key; references now follow the customer into Submission, Policy, Evidence, search, and breadcrumb displays.
- Role-aware contextual search across every applicable collection/workbench: owner-scoped Submissions, Evidence, Policies, and Claims; operational Underwriting referral and Claims adjudication queues; and personal/team Notifications. Role-specific status, date, risk, priority, assignment, evidence, coverage, incident, and read-state filters never widen the caller's existing owner/team/policy scope.
- Evidence document contracts (`Required`, `Optional`, `NarrativeOnly`) enforced in the Underwriting command handler and explained in both Customer and Underwriter interfaces. Automatically projected control evidence is Required; manual Underwriting requests choose deliberately.
- Shared semantic breadcrumbs and friendly browser-local timestamps on deeper customer and operational pages. Draft creation and Evidence response now have dirty-aware Cancel/discard flows that clear local form, mutation, idempotency, and selected-file state without deleting durable audit records.
- Role-aware search/navigation verification passed with a zero-warning solution build; 209 unit tests; 274 Docker-backed integration tests plus 3 intentional external-service skips; four clean EF pending-model checks; and frontend TypeScript, ESLint, production build, and all 99 tests. Local CI artifact: `TestResults/local-ci-20260714-171307.zip`.

- Central customer-safe error boundary: API business conflicts use stable Problem Details codes and correlation IDs; unhandled failures are centrally logged and converted to safe `500` responses; every frontend feature API uses a Zod-validated `ApiError`; page, login, role-loading, and React-render failures no longer expose raw JSON, exception, provider, or database text. A support ID remains available for developer investigation.
- Exact owner-scoped evidence and quote reads: filterable cursor-paged evidence summaries, `/api/v1/evidence-requests/{id}` detail with document loading only on detail, `/evidence-requests/:id`, exact immutable quote history at `/submissions/:submissionId/quotes/:quoteId`, compatibility redirect from `/evidence`, and `404` isolation for another owner's resources.
- Production observability contract: low-cardinality request count/duration signals, structured request-outcome and unhandled-exception event IDs, JSON console logging for Production/Aws, a disabled-by-default privacy-safe browser telemetry bridge, and a Terraform-owned CloudWatch/RUM runbook covering log groups, retention, metric filters, alarms, notification targets, sampling, access, and privacy.
- Notification event snapshots now retain quote version/premium/expiry and evidence request title/category/due date where available, producing descriptive inbox titles and exact evidence/quote deep links without read-time cross-context queries.
- Customer error and notification hardening verification passed with a zero-warning solution build; 206 unit tests; 269 Docker-backed integration tests plus 3 intentional external-service skips; four clean EF pending-model checks; and frontend TypeScript, ESLint, production build, and all 96 tests. Local CI artifact: `TestResults/local-ci-20260714-003907.zip`.

- Cyber control assurance and reassessment: quote generation now requires a named, versioned customer attestation; persists five detailed control assertions (MFA, EDR, backup/recovery, incident response, sensitive data); applies deterministic risk-based evidence rules; separates commercial Quote status from `SelfAttested` / `EvidenceRequired` / `Verified` / `Rejected` assurance; and blocks acceptance while required evidence is incomplete.
- Outbox-driven assurance hand-off: `QuoteGenerated` idempotently projects category-specific requests into the Underwriting module, while satisfied/remediation evidence-review events project back into Quoting through a separate replay ledger. Cross-context effects remain id-only, eventually consistent, and free of cross-schema writes.
- Immutable quote reassessment: a genuine pre-acceptance control change creates version N+1, links `SupersedesQuoteId`, marks N `Superseded`, repeats attestation, and requires evidence for claimed improvements. Accepted/bound changes remain future endorsement/renewal work.
- Advisory evidence assessment: clean uploads receive a versioned deterministic plausibility assessment and human-review consistency status. Findings are persisted and visible to underwriters but cannot approve evidence, change pricing, or make an insurance decision.
- Cyber-control assurance verification passed with a zero-warning solution build, 203 unit tests, 266 Docker-backed integration tests plus 3 intentional external-service skips, four clean EF pending-model checks, and 89 frontend tests. Local CI artifact: `TestResults/local-ci-20260712-050920.zip`.
...
- App-wide transient feedback audit: seven true success confirmations across Submission, Evidence, and Underwriting now use the shared five-second accessible fade/unmount behavior; durable policy/quote/document results, timelines, audit records, warnings, errors, and empty states remain persistent. Delete/withdraw lifecycle confirmations share the styled focus-managed modal and clearly explain Draft deletion versus retained Submitted audit history.
...
- **Claims bounded context (Phase 3, CM1–CM8) — the post-bind claim lifecycle**, built in parallel on `feat/claims-context` and merged as one consolidation. New `src/Modules/Claims/{Domain,Application,Infrastructure}` owning a `claims` PostgreSQL schema (`ClaimsDbContext`, five additive migrations, `claims-db` readiness check) and a fourth outbox source (`ClaimsOutboxSource`). The `Claim` aggregate enforces the lifecycle (Filed → UnderReview → InformationRequested → Accepted/Denied → Closed) with a file-time policy snapshot and a `Version` optimistic-concurrency token.
  ...
  - **CM3 documents:** scan-gated claim documents (store → quarantine-scan → clean-only download; the evidence trust model).
  - **CM4 financials:** claimed amount, adjuster-only **reserve** (append-only audited history, confidential to the claimant), paid amount.
  - **CM5 decision:** accept (settlement) / deny (reason) / close with three domain-enforced guardrails (no decision without assignment; settlement ≤ limit net of retention; denial requires a reason); append-only decision audit; lifecycle events.
  - **CM6 notifications:** seven claim message types mapped via the M40 registry (zero dispatcher changes) + a `claims-operations` team inbox.
  - **CM7 frontend:** the `features/claims` slice — claimant wizard/list/detail + adjuster workbench; `RequireRole` guard; dashboard cards.
  - **Post-CM8 hardening:** split-query fixes, pure-SQL queue projection, reserve auto-release on close, and **server-authoritative roles** via `GET /api/v1/me` (the SPA reads roles from the API, not the token — provider-neutral for the future Cognito option; the "roles on the ID token" step is obsolete).
  - Docs consolidated into the Tier-1 living docs: new Encyclopedia **Chapter 12 (Flow: Claims)**, Encyclopedia Ch 3/5/10 updates, Build History **Era 7**, user-roles, guides, README/docs-map. Per-milestone records archived under `docs/claims/`.
...
- Milestone 44.5 - Referral Queue Hardening: referral assignment is now a guarded claim — the domain rejects a second underwriter, and a `Version` optimistic-concurrency token on `quote_referral_operations` (migration `AddReferralOperationConcurrencyVersion`) catches true races at save time; the losing writer receives HTTP 409 and the workbench refetches to show the real assignee (same-user re-clicks stay idempotent).
- The underwriting referral queue read (`ListQuoteReferralsQuery`) is now cached shared for 10 seconds (`underwriting:referral-queue:v1`), with read-your-writes preserved by `ReferralQueueCacheInvalidationFilter` — an API-edge action filter that evicts the entry after every successful queue-affecting write (workbench operations/decisions/evidence, owner evidence responses, quote creation). Reads and failed writes never evict; Worker-side projection changes ride the TTL (already eventually consistent).
- Tests: domain assignment guard/idempotency/version tests, a two-context concurrency race test, an endpoint double-assign 409 test, and invalidation-filter tests; the entire pre-existing endpoint suite passed unchanged with the cache active.
...
- Quote reassessment is cancellable local UI state until creation succeeds. Clean cancellation restores immediately; dirty cancellation uses the accessible discard modal; attestation and mutation state reset; and `Create reassessment` remains disabled until at least one canonical control assertion changes. The server still enforces `quote.reassessment.no_changes`.
- The owner Evidence page now renders bounded summaries rather than every response form and document collection. Cursor ordering includes due time, requested time, and evidence request ID so equal-timestamp requests cannot be skipped.
...
- Reassessment self-service governance no longer treats the original Quote as a prior reassessment. The first valid change creates Quote version 2 immediately; only a successful reassessment starts the 30-minute cooldown. Cooldown attempts now return retry guidance without calling the rating provider or creating a pending Underwriter review, while rolling/lifetime allowance overflow still follows the audited approval path.
- Reassessment cooldown correction verification passed with a zero-warning solution build; 226 Unit tests; 284 standalone Integration tests plus 4 intentional service skips; four clean EF pending-model checks; and frontend TypeScript, ESLint, production build, and all 111 tests. Docker-backed local CI passed 226 Unit tests, 285 Integration tests plus 3 expected skips, clean frontend install/build/lint/tests and API smoke checks, then cleaned PostgreSQL and Redis. Artifact: `TestResults/local-ci-20260716-055648.zip`.
...
- Quote generation is now durable across refreshes: submission detail returns the latest quote, repeated quote-generation requests return the existing quote instead of creating another notification, notifications link back to the related submission, and quote-field help controls now show on hover/focus instead of staying open after click.
...
- The dashboard no longer renders the local-only developer session check or session status debug panels now that role-aware navigation and Auth0 consent recovery are in place.
